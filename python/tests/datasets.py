    ...
    contract = taco.Contract(
        ...
        metadata=taco.MetadataSchema(
            taco.Level(
                ...
                provenance=taco.metadata.sample.SplitProvenance,
            )
        ),
    )
    samples = tuple(
        taco.Sample(
            metadata=taco.Metadata(
                ml=taco.metadata.sample.Split(split="train" if index == 0 else "validation"),
                provenance=taco.metadata.sample.SplitProvenance(
                    original="train" if index == 0 else "valid",
                    noleak="train" if index == 0 else "validation",
                    noleak_rule="example/inherit-v1",
                ),
            ),
